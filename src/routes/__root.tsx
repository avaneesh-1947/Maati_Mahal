...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "twitter:title", content: `${restaurant.name} | Family Restaurant in ${restaurant.city}` },
      { name: "twitter:description", content: `Authentic village-inspired pure vegetarian cuisine, traditional thalis & Bati Chokha on NH56 in ${restaurant.city}.` },
      { name: "twitter:image", content: absoluteUrl("/favicon.jpeg") },
      { name: "theme-color", content: "#6E1F2A" },
      { property: "og:image", content: absoluteUrl("/favicon.jpeg") },
      { name: "geo.region", content: "IN-UP" },
      { name: "geo.placename", content: `${restaurant.city}, ${restaurant.state}` },
      { name: "geo.position", content: `${restaurant.geo.latitude};${restaurant.geo.longitude}` },
      { name: "ICBM", content: `${restaurant.geo.latitude}, ${restaurant.geo.longitude}` },
    ],
    links: [
